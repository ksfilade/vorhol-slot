...
const ROWS_NUMBER = 4;
const REELS_NUMBER =10;
const REEL_SPPED = 2500;
// Create a new PixiJS application with TypeScript
(async () => {
    ...
    const margin = (app.screen.height - SYMBOL_SIZE * ROWS_NUMBER) / 2;

    reelContainer.y = margin;
    reelContainer.x = Math.round(app.screen.width - REEL_WIDTH * REELS_NUMBER);
    ...
    bottom.drawRect(0, SYMBOL_SIZE * ROWS_NUMBER + margin, app.screen.width, margin);
    bottom.endFill();
    ...
    function startPlay() {
        ...
        for (let i = 0; i < reels.length; i++) {
            const r = reels[i];
            const extra = Math.floor(Math.random() * REELS_NUMBER);
            const target = r.position + 10 + i * 5 + extra;
            const time = REEL_SPPED + i * 600 ;
    ...
    const tweening: Array<{
        ...
        time: number;
        change?: (tween: any) => void;
        complete?: (tween: any) => void;
        start: number;
    }> = [];
